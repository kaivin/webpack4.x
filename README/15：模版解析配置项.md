...
* 这里我配置了 `pages` `components` 两个路径别名，这是比较常见的两个文件夹，可以现在 `src` 文件夹下创建这两个文件夹，以后用得到~ 一个页面文件夹，一个组件文件夹，当用到这两个文件夹内的任何文件，都可以直接使用 别名 + 别名指定的路径下文件的路径就可以引用，无论该路径多么复杂，再也不会出现一大串 `../../` 了~
